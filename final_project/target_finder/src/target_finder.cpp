...
geometry_msgs::Point targetPositions;
...
class TargetFinder
{
    ...
    : it_(nh_)
  {
      ...
      rgb_image_sub_ = it_.subscribe(color_topic, 1,
         &TargetFinder::imageCb, this);
      depth_image_sub_ = it_.subscribe(depth_topic, 1,
         &TargetFinder::depthCb, this);
      detection_pub = it_.advertise(detection_result, 1);
      target_position_pub = nh_.advertise<geometry_msgs::Point>(pub_name, 1);
  }
  ...
  void depthCb(const sensor_msgs::ImageConstPtr& msg)
  {
      ...
      if(target_found)
      {
          ...
          if(target_distance != 0)
          {
              ...
              targetPositions.x = targetPositionX;
              targetPositions.y = targetPositionY;
              target_position_pub.publish(targetPositions);
...
int main(int argc, char** argv)
{
        ...
        parameterServer.getParam("color_topic",color_topic);
        parameterServer.getParam("depth_topic",depth_topic);
        parameterServer.getParam("detection_result",detection_result);
        parameterServer.getParam("publisher_name",pub_name);
